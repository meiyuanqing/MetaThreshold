...
Deriving Alves's method threshold by meta-analysis.
computes the variance of high level threshold: var(T)=var(cX)=c^2×var(X)=[E(T)/E(X)]^2×var(X)
"""


def Alves_meta(t_dir="F:\\NJU\\MTmeta\\experiments\\unsupervised\\trainingData\\",
              m_dir="F:\\NJU\\MTmeta\\experiments\\unsupervised\\AlvesThresholdMinified\\"):
    ...
    df = pd.read_csv(meta_dir + "Alves_Threshold_minified.csv", keep_default_na=False, na_values=[""])
    ...
    for metric in metric_names:
        ...
        try:
            ...
            with open(meta_dir + "Alves_metaThresholds.csv", 'a+', encoding="utf-8", newline='') as f:
                writer_f = csv.writer(f)
                if os.path.getsize(meta_dir + "Alves_metaThresholds.csv") == 0:
                    writer_f.writerow(["metric", "Alves_metaThreshold", "Alves_metaThreshold_stdError", "LL_CI", "UL_CI",
...
if __name__ == '__main__':
    ...
    print("The __name__ is ", __name__, ". This is end of AlvesMeta.py!")
