...
Date : 2021/4/4
Time: 9:29
File: OliveiraMeta.py
HomePage : http://github.com/yuanqingmei
Email : dg1533019@smail.nju.edu.cn

Deriving Oliveira's method threshold by meta-analysis.
computes the variance of high level threshold: var(T)=var(cX)=c^2×var(X)=[E(T)/E(X)]^2×var(X)
"""


def Oliveira_meta(t_dir="F:\\NJU\\MTmeta\\experiments\\unsupervised\\trainingData\\",
              m_dir="F:\\NJU\\MTmeta\\experiments\\unsupervised\\OliveiraThresholdMinified\\"):
    ...
    df = pd.read_csv(meta_dir + "Oliveira_Threshold_minified.csv", keep_default_na=False, na_values=[""])
    ...
    for metric in metric_names:
        # print(metric.split('_')[0], "       ", metric[-6:])
        # chooses the high level value as the threshold
        # if metric[-6:] != "_85.0%":
        #     continue
        if not ((metric[-6:] == "_85.0%") or (metric[-6:] == "_90.0%") or (metric[-6:] == "_95.0%")
                or (metric[-7:] == "_100.0%")):
            continue
        print(metric.split('_')[0])
        # There are no more than 10 values of DCAEC and DCMEC metric, which are greater than 0. drop it
        if (metric[:-6] == "DCAEC") or (metric[:-6] == "DCMEC") or (metric.split('_')[0] == "FCAEC")\
                or (metric.split('_')[0] == "FCMEC") or (metric.split('_')[0] == "FMMEC")\
                or (metric.split('_')[0] == "IFCAIC") or (metric.split('_')[0] == "IFCMIC")\
                or (metric.split('_')[0] == "IFMMIC"):
            continue
        ...
        for i in range(len(df["fileName"])):
            ...
            mean_metric = df_metric[metric.split('_')[0]].mean()
            variance_metric = df_metric[metric.split('_')[0]].var()
        ...
        try:
            ...
            with open(meta_dir + "Oliveira_metaThresholds.csv", 'a+', encoding="utf-8", newline='') as f:
                writer_f = csv.writer(f)
                if os.path.getsize(meta_dir + "Oliveira_metaThresholds.csv") == 0:
                    writer_f.writerow(["metric", "Oliveira_metaThreshold", "Oliveira_metaThreshold_stdError", "LL_CI",
...
if __name__ == '__main__':
    ...
    print("The __name__ is ", __name__, ". This is end of OliveiraMeta.py!")
