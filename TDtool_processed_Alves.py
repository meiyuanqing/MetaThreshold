...
working_dir = "F:\\NJU\\MTmeta\\experiments\\unsupervised\\AlvesThreshold\\"
result_dir = "F:\\NJU\\MTmeta\\experiments\\unsupervised\\AlvesThresholdMinified\\"
...
for line in lines:
    ...
    df = pd.read_csv(working_dir + file[:-4] + "\\Alves's Method Output\\Final-Result.csv", sep=';',
                     keep_default_na=False)

    df['metric_label'] = df.apply(lambda x: x['Metric'] + "_" + x['Lable'], axis=1)
    Alves_Threshold_minified_fieldname = ["fileName"]
    for i in range(len(df['metric_label'])):
        Alves_Threshold_minified_fieldname.append(df.loc[i, 'metric_label'])

    # reads a csv file and stores as dataframe
    with open(working_dir + file[:-4] + "\\Alves's Method Output\\Final-Result.csv",
              'r', encoding="ISO-8859-1") as df_input, \
            open(result_dir + "Alves_Threshold_minified.csv", 'a+', encoding="utf-8", newline='') as file_output:

        writer = csv.writer(file_output)
        print("the size of file_output is ", os.path.getsize(result_dir + "Alves_Threshold_minified.csv"))
        if os.path.getsize(result_dir + "Alves_Threshold_minified.csv") == 0:
            writer.writerow(Alves_Threshold_minified_fieldname)
        ...
        Alves_Threshold_minified_row = [file[:-4]]
        for i in range(len(df['metric_label'])):
            ...
            if df.loc[i, 'Metric'] in decimal_metric:
                ...
                Alves_Threshold_minified_row.append(df.loc[i, 'Value'] * 0.01)
                continue
            Alves_Threshold_minified_row.append(df.loc[i, 'Value'])

        writer.writerow(Alves_Threshold_minified_row)
