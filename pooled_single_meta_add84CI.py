...
"""
Author : Yuanqing Mei
Date : 2021/4/24;update on 2021/8/9;
Time: 15:51
File: pooled_single_meta.py
HomePage : http://github.com/yuanqingmei
Email : dg1533019@smail.nju.edu.cn

Deriving each method threshold by meta-analysis:
           four unsupervised and five supervised methods.
                     Four unsupervised methods: Alves, Vale, Ferreira, Oliveira;
                     Five supervised methods: Bender, ROC, BPP, MFM, GM.
根据第一种思路是所的方法产生的阈值后，方差统一用项目内的均值和方差来做元分析，样本量为9*9=81；

计算每一种方法上，每个度量在训练集的所有系统上的阈值和方差进行汇总元分析，个数只有9个。

计算有监督和无监督学习方法下阈值元分结果和敏感性分析结果，并为计算各自的AUC及敏感性结果服务
...
def pooled_all_meta(t_dir="F:\\NJU\\MTmeta\\experiments\\unsupervised\\trainingData\\",
    ...
    def random_effect_meta_analysis(effect_size, variance):
        ...
        # 20210809 adds the 84% CI for the summary effect
    ...
    for metric in metric_names:
        ...
        try:
            ...
            with open(meta_dir + "single_method\\unsupervised_meta_thresholds.csv", 'a+', encoding="utf-8", newline='') as f:
                writer_f = csv.writer(f)
                if os.path.getsize(meta_dir + "single_method\\unsupervised_meta_thresholds.csv") == 0:
                    writer_f.writerow(
    ...
    nine_thresholds.to_csv(meta_dir + "single_method\\unsupervised_method_thresholds.csv", encoding="ISO-8859-1", index=False, mode='w')
...
if __name__ == '__main__':
    ...
    print("The __name__ is ", __name__, ". This is end of pooled_single_meta.py!",
          "\nThe execution time of pooled_single_meta.py script is ", execution_time)
